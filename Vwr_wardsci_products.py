...
if __name__ == '__main__':
    ...
    content = soup.find('ul', class_='a-z_categorylist')
    inner_content = content.find_all('a')[2]
    product_category = inner_content.text.strip()
    main_url = f'{base_url}{inner_content["href"]}'
    print(f'main_url---------------->{main_url}')
    inner_request = get_soup(main_url, headers)
    '''PRODUCT URL'''
    url_href = inner_request.find_all('div', class_='search-item row')[:5]
...
#             main_url = 'https://us.vwr.com/store/category/antibodies/3617058'
#             print(f'main_url---------------->{main_url}')
