...
if __name__ == '__main__':
    ...
    for single_content in main_content.find_all('li', class_=re.compile('level0.*?'))[:1]:
        main_link = single_content.find_all('li', class_=re.compile('level2.*?'))[2]
        ...
        product_content = inner_request.find_all('div', class_='product details product-item-details')[:10]
        for single_product in product_content:
            ...
            if 'spConfig' in str(capacity):
                ...
                replace_tag = str(content_replace).replace('"options":', '').replace(',"template":', '').replace(',"position":', '').strip()
                ...
                for single_data in inner_json:
                    ...
                    match_str = str(single_data['products']).replace(']', '').replace('[', '').replace("'", '').strip()
                    try:
