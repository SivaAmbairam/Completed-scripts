...
def scrape_category(category_url, headers, base_url):
    ...
    for i in range(page_data):
        ...
        json_soup = get_json_response(json_url, headers)
        content_json = json_soup.get('Items', [])
        for single_contents in content_json[:10]:
            product_url = f'{base_url}{single_contents["Url"]}'
            product_request = scrape_product(product_url, headers)
            if not product_request:
                logging.error(f"Failed to scrape product page: {product_url}")
...
def save_product_data(product_dict):
    if product_dict is not None:
        ...
        print(product_dict.get('Flinn_product_url', 'N/A'))
...
if __name__ == '__main__':
    ...
    data = soup.find_all('li', class_='b-main-nav-inner-content__hidden-content__links-list-item_first-level')[1]
    content = data.find('a')
    ...
    if inner_req.find('a', class_='b-categories__category__link'):
        inner_category = inner_req.find_all('a', class_='b-categories__category__link')[0]
        category_url = f'{base_url}{inner_category["href"]}'
        if inner_category.find('h3', class_='b-categories__category__name'):
            product_sub_category = inner_category.find('h3', class_='b-categories__category__name').text.strip()
        else:
